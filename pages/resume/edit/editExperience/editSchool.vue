				...
				<view class="resume-item-content">
					<view class="content-item" v-for="(item,index) in school" :key="index">
						<uni-forms :value="item" ref="form1">
							...
							<uni-forms-item label="在校时间" name="name">
							...
							<uni-forms-item label="学历" name="edu_background">
							...
							<uni-forms-item label="是否统招" name="background_type">
...
<script>
	...
	export default {
		...
		onLoad: function (option) {
			...
			this.scItem = item
			console.log(item)
			this.school[0].start_at = this.scItem.start_at
			this.school[0].end_at = this.scItem.end_at
			this.school[0].description = this.scItem.description
			this.school[0].schName = this.scItem.school_name
			this.school[0].edu_background = this.scItem.edu_background
			this.school[0].background_type = this.scItem.background_type
		
			let naindex = 0
			let stindex = 0
			this.eduLevel.forEach(item=>{
				if(item.key === this.scItem.edu_background){
			...
			this.eduType.forEach(item=>{
				if(item.key === this.scItem.background_type){
					this.naindex = naindex
					console.log(this.naindex)
				}else{
					naindex = naindex+1
				}
			})
		...
		methods:{
			...
			deleted(){
				resume.delResumeSchool({
					resume_id:this.scItem.resume_id,
					school_id:this.scItem.id
				}).then(res=>{
					uni.showToast({
						title:res.msg ,
					})
