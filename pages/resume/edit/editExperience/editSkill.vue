			...
			<view class="resume-item-content">
				<view class="content-item" v-for="(item,index) in skill" :key="index">
					<uni-forms :value="item">
						...
						<!-- <lb-picker ref="picker" :name="'picker'+skindex" v-model='list'></lb-picker> -->
					</uni-forms>
...
<script>
	...
	export default {
		...
		onLoad: function(option) {
			...
			this.skItem = item
			console.log(item)
			this.skill[0].skillName = this.skItem.name
			this.skill[0].proficiency = this.skItem.skilled
			this.skill[0].use_time = this.skItem.use_time
			let xindex = 0
			this.skillLevel.forEach(item=>{
				if(item.key === this.skItem.skilled){
					this.skindex = xindex
					console.log(this.skindex)
				}else{
					xindex = xindex+1
				}
			})
		...
		methods: {
			...
			skbindPickerChange(e) {
				...
				console.log(this.skillLevel[e.target.value].key)

			},
			deleted() {
				resume.delResumeSkill({
					resume_id: this.skItem.resume_id,
					skill_id: this.skItem.id
				}).then(res => {
					uni.showToast({
						title: res.msg,
					})
