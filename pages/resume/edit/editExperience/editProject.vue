...
<script>
	...
	export default {
		...
		onLoad: function (option) {
			...
			this.proItem = item
			this.projects[0].start_at = this.proItem.start_at
			this.projects[0].end_at = this.proItem.end_at
			this.projects[0].description = this.proItem.description
			this.projects[0].name = this.proItem.name
		},
		...
		methods:{
			...
			deleted(){
				resume.delResumeProject({
					resume_id:this.proItem.resume_id,
					project_id:this.proItem.id
				}).then(res=>{
					uni.showToast({
						title:res.msg ,
					})
