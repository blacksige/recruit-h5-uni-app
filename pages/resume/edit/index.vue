<template>
	<view class="add-box">
		...
		<view class="resume-item">
			...
			<view class="resume-item-content">
				<uni-forms :value="resumeData" ref="form" :rules="Rules">
					...
					<uni-forms-item label="性别" name="sex" :required='true' e>
					...
					<uni-forms-item label="期望地点" name="expect_city_id" :required='true'>
						<view class="time-pickers">
							<picker class="time-picker" :value="expect_city_idIndex1" @change="cityChange1" :range="countryList" range-key="name">
								...
								<view class="tipText" v-else>{{resumeData.region_province}}</view>
							</picker>
							<view style="line-height: 36px;color: #808080">-</view>
							<picker class="time-picker" :value="expect_city_idIndex2" @change="cityChange2" :range="cityList" range-key="name"
							 :disabled="!ifCityShow">
								...
								<view class="tipText" v-else>{{resumeData.region_city}}</view>
							</picker>
						</view>
					</uni-forms-item>
				</uni-forms>
				<!-- 	<button type="primary" @click="nextAction" class="submit-btn" >提交，下一步</button> -->
			</view>
		...
		<view class="resume-item">
			...
			<view class="resume-item-content">
				<view class="content-item" v-for="(item,index) in work" @click="editExperiencewr(item)" :key="index">
					<view class="name_date">
						<view class="this_name">{{item.company_name}}</view>
						<view class="this_date">{{item.start_at}}&nbsp;至&nbsp;{{item.end_at}}<text class="icon-dayu1 iconfont"></text></view>
					</view>
		...
		<view class="resume-item">
			...
			<view class="resume-item-content">
				<view class="content-item" v-for="(item,index) in projects" @click="editExperiencepr(item)" :key="index">
					<view class="name_date">
		...
		<view class="resume-item">
			...
			<view class="resume-item-content">
				<view class="content-item" v-for="(item,index) in school" @click="editExperiencesc(item)" :key="index">
		...
		<view class="resume-item">
			...
			<view class="resume-item-content">
				<view class="content-item" v-for="(item,index) in skill" @click="editExperiencesk(item)" :key="index">
					<view class="space_bw">
						<view style="margin-top: 12rpx;"><span>{{item.name}}</span></view><text class="icon-dayu1 iconfont"></text>
					</view>
	...
	export default {
		data() {
			...
			return {
				...
				ifexShow: false,
				ifCityShow: false,
				imgUrl: "../../../static/imgs/avatar-violet.png",
				// imgUrl2: "",
				other: currentDate,
		...
		onLoad() {
			...
			resume.getResumeDetail({
				...
				if (res.code == 200) {
					let arr = res.data[0]
					console.log(arr)
					this.resumeData.id = arr.id
					this.resumeData.name = arr.name
					this.resumeData.avatar = arr.avatar
					this.imgUrl = arr.avatar
					this.resumeData.expect_city_id = arr.expect_city_id
					this.resumeData.expect_province_id = arr.expect_province_id
					this.resumeData.expect_position_id = arr.expect_position_id
					this.resumeData.sex = arr.sex
					this.resumeData.edu_background = arr.edu_background
					let sexIndex = 0
					let expect_position_idIndex = 0
					let stindex = 0
					this.sexList.forEach(item => {
						if (item.key == this.resumeData.sex) {
					...
					this.eduLevel.forEach(item => {
						console.log(item.key)
						console.log(this.resumeData.edu_background)
						if (item.key == this.resumeData.edu_background) {
							this.stindex = stindex
							console.log(this.stindex)
						} else {
							stindex = stindex + 1
						}
					})
					this.positionList.forEach(item => {
						if (item.id == this.resumeData.expect_position_id) {
							this.expect_position_idIndex = expect_position_idIndex
							console.log(this.expect_position_idIndex)
						} else {
							expect_position_idIndex = expect_position_idIndex + 1
						}
					})
					this.resumeData.mobile = arr.mobile
					this.resumeData.expect_salary_m_start = arr.expect_salary_m_start
					this.resumeData.expect_salary_m_end = arr.expect_salary_m_end
					this.resumeData.region_province = arr.region_province
					this.resumeData.region_city = arr.region_city
		...
		methods: {
			...
			edbindPickerChange(e) {
				...
				this.resumeData.edu_background = this.eduLevel[e.target.value].key
			},
			...
			addWorkExperience() {
				...
				// })
				uni.navigateTo({
					url: '../add/addExperience/addWork?id=' + this.thisResume_id,
				});
			},
			addProjectExperience() {
				...
				// })
				uni.navigateTo({
					url: '../add/addExperience/addProject?id=' + this.thisResume_id,
				});
			},
			addSchoolExperience() {
				// this.school.push({
				...
				// })
				uni.navigateTo({
					url: '../add/addExperience/addSchool?id=' + this.thisResume_id,
				});
			},
			addSkill() {
				// this.skill.push({
				...
				// })
				uni.navigateTo({
					url: '../add/addExperience/addSkill?id=' + this.thisResume_id,
				});
			},
			...
			editExperiencewr(item) {
				console.log(item)
				uni.navigateTo({
					url: './editExperience/editWork?item=' + encodeURIComponent(JSON.stringify(item)),
				});
			},
			editExperiencepr(item) {
				console.log(item)
				uni.navigateTo({
					url: './editExperience/editProject?item=' + encodeURIComponent(JSON.stringify(item)),
				});
			},
			editExperiencesc(item) {
				console.log(item)
				uni.navigateTo({
					url: './editExperience/editSchool?item=' + encodeURIComponent(JSON.stringify(item)),
				});
			},
			editExperiencesk(item) {
				console.log(item)
				uni.navigateTo({
					url: './editExperience/editSkill?item=' + encodeURIComponent(JSON.stringify(item)),
				});
			},
