<template>
	<view :class="['lb-picker', inline ? 'lb-picker-inline' : '']">
		...
		<!-- 遮罩层 -->
		<view v-if="visible && showMask && !inline" :class="['lb-picker-mask', animation ? 'lb-picker-mask-animation' : '']"
		 :style="{
			...
			<view v-if="showHeader" class="lb-picker-header">
				...
				<view class="lb-picker-header-actions">
					...
					<view class="lb-picker-action lb-picker-action-cancel" @tap.stop="handleCancel">
						...
						<text v-else class="lb-picker-action-cancel-text" :style="{ color: cancelColor }">{{ cancelText }}</text>
					</view>
					...
					<view class="lb-picker-action lb-picker-action-confirm" @tap.stop="handleConfirm">
						...
						<text v-else class="lb-picker-action-confirm-text" :style="{ color: confirmColor }">{{ confirmText }}</text>
					</view>
				</view>
        ...
        safeAreaInsetBottom ? 'lb-picker-content-safe-buttom' : ''
				...
				<view class="lb-picker-content-main" :style="{ height: pickerContentHeight }">
					...
					<!-- 单选 -->
					<selector-picker v-if="mode === 'selector' && !loading && !isEmpty" :ref="mode" :value="value" :list="list" :mode="mode"
					 :props="pickerProps" :height="pickerContentHeight" :inline="inline" :column-style="columnStyle"
					 :active-column-style="activeColumnStyle" :align="align" :press-enable="pressEnable" :press-time="pressTime"
					 :formatter="formatter" @change="handleChange">
					</selector-picker>

					<!-- 多列联动 -->
					<multi-selector-picker v-if="mode === 'multiSelector' && !loading && !isEmpty" :ref="mode" :value="value" :list="list"
					 :mode="mode" :level="level" :visible="visible" :props="pickerProps" :height="pickerContentHeight" :inline="inline"
					 :column-style="columnStyle" :active-column-style="activeColumnStyle" :align="align" :press-enable="pressEnable"
					 :press-time="pressTime" :formatter="formatter" @change="handleChange">
					</multi-selector-picker>

					<!-- 非联动选择 -->
					<unlinked-selector-picker v-if="mode === 'unlinkedSelector' && !loading && !isEmpty" :ref="mode" :value="value"
					 :list="list" :mode="mode" :visible="visible" :props="pickerProps" :height="pickerContentHeight" :inline="inline"
					 :column-style="columnStyle" :active-column-style="activeColumnStyle" :align="align" :press-enable="pressEnable"
					 :press-time="pressTime" :formatter="formatter" @change="handleChange">
					</unlinked-selector-picker>

					<!-- 日期选择 -->
					<date-selector-picker v-if="mode === 'dateSelector' && !loading" :ref="mode" :value="value" :mode="mode" :visible="visible"
					 :height="pickerContentHeight" :inline="inline" :column-style="columnStyle" :active-column-style="activeColumnStyle"
					 :align="align" :formatter="formatter" :format="format" :display-format="displayFormat" :start-date="startDate"
					 :end-date="endDate" :default-time-limit="defaultTimeLimit" :is-show-chinese="isShowChinese" :ch-config="pickerChConfig"
					 :filter="filter" @change="handleChange">
