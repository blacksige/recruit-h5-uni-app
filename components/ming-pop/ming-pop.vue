<template>
	...
	<view :class="direction==='center'?'centers':''" v-if="direction==='center'?open:true">
		<view class="product-window" :style="{width:width+'%'}" :class="(open ? 'on' : '')+' '+direction">
			...
			<!-- #endif -->
			<image src="../../static/ming-pop/close.png" mode="" :class="(direction!=='below'&&direction!=='center')?'iconfont-h5':''"
			 class="iconfont" @click="open=false" v-if="is_close"></image>
			<slot></slot>
		</view>
...
<style lang="scss" scoped>
	...
	.product-window {
		...
		border-radius: 15rpx;
		padding: 50rpx 40rpx;
		transition: all .6s cubic-bezier(.25, .5, .5, .9);
		-webkit-transition: all .6s cubic-bezier(.25, .5, .5, .9);
	}
